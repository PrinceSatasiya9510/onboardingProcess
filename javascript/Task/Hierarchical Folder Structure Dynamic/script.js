...
function folderDelete(folderId) {
    ...
    let findIndexOfDeleteFolder = parentFolder["childrens"].findIndex(item => item.id == folderId)
...
function popup(response, folderEditIndex) {
    ...
    if (!editIndex) {
        folderCreateButton.setAttribute("onclick", `createNewFolder(${response},${folderEditIndex})`)
